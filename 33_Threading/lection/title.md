...
Щоб уникнути такої поведінки, використовується метод ```join()```, який приєднує тред до основного потоку(якщо в дужках нічого не вказано). Як наслідок - основний потік продовжить свою роботу лише після того, як дочекається завершення приєднаного потоку.
